...
Uso:
    APP_CRYPTO_KEY=<base64-32-bytes> python scripts/build_seed.py <caminho-do-xlsx> [n]

Default: 300 amostras, output em src/main/resources/db/migration/V3__seed_real_data.sql
"""
...
def main():
    if len(sys.argv) < 2:
        print(f"Uso: python {sys.argv[0]} <caminho-do-xlsx> [n_amostras]")
        sys.exit(1)
    ...
    out_path = Path(__file__).resolve().parent.parent / "src" / "main" / "resources" / "db" / "migration" / "V3__seed_real_data.sql"
